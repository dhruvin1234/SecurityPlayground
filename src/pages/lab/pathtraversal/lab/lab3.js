import React, { useState } from "react";
import "./lab3.css";

const fileSystem = {
  "public/readme.txt": "Welcome to Security Playground! This is the readme.",
  "secret.txt": "This is a secret message stored in the file.",
  "../../etc/passwd": `
root:x:0:0:root:/root:/bin/bash
daemon:x:1:1:daemon:/usr/sbin:/usr/sbin/nologin
bin:x:2:2:bin:/bin:/usr/sbin/nologin
sys:x:3:3:sys:/dev:/usr/sbin/nologin
sync:x:4:65534:sync:/bin:/bin/sync
games:x:5:60:games:/usr/games:/usr/sbin/nologin
man:x:6:12:man:/var/cache/man:/usr/sbin/nologin
lp:x:7:7:lp:/var/spool/lpd:/usr/sbin/nologin
mail:x:8:8:mail:/var/mail:/usr/sbin/nologin
news:x:9:9:news:/var/spool/news:/usr/sbin/nologin
uucp:x:10:10:uucp:/var/spool/uucp:/usr/sbin/nologin
proxy:x:13:13:proxy:/bin:/usr/sbin/nologin
www-data:x:33:33:www-data:/var/www:/usr/sbin/nologin
backup:x:34:34:backup:/var/backups:/usr/sbin/nologin
dhcp:x:101:101:DHCP server:/nonexistent:/usr/sbin/nologin
user:x:1000:1000:User:/home/user:/bin/bash
`,
  "../../home/user/.ssh/id_rsa": `
-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----
`,
  "../../var/www/html/index.php": `
<?php echo "Welcome to the vulnerable site!"; ?>
`,
};

const Lab3 = () => {
  const [filePath, setFilePath] = useState("");
  const [fileContent, setFileContent] = useState("");

  const handleSubmit = (e) => {
    e.preventDefault();

    // Decode input and simulate null byte truncation
    let rawPath = decodeURIComponent(filePath.trim());
    const nullByteIndex = rawPath.indexOf("\x00");

    if (nullByteIndex !== -1) {
      rawPath = rawPath.substring(0, nullByteIndex);
    }

    // Simulate basic validation (only .txt allowed)
    if (!filePath.endsWith(".txt")) {
      setFileContent("❌ Only .txt files are allowed.");
      return;
    }

    // Check file from our mock file system
    if (fileSystem[rawPath]) {
      setFileContent(fileSystem[rawPath]);
    } else {
      setFileContent("❌ File not found or access denied.");
    }
  };

  return (
    <div className="lab3-container">
      <div className="lab3-box">
        <h1 className="lab3-title">🧨 Traversal with Null Byte</h1>
        <p className="lab3-description">
          Try bypassing extension filter using a null byte (<code>%00</code>).<br />
          Example: <code>../../etc/passwd%00.txt</code>
        </p>
        <form onSubmit={handleSubmit} className="lab3-form">
          <input
            type="text"
            className="lab3-input"
            placeholder="Enter file path (e.g., ../../etc/passwd%00.txt)"
            value={filePath}
            onChange={(e) => setFilePath(e.target.value)}
          />
          <button type="submit" className="lab3-button">
            Read File
          </button>
        </form>
        <div className="lab3-output">
          <div className="lab3-result-box">
            <pre>{fileContent}</pre>
          </div>
        </div>
      </div>
    </div>
  );
};

export default Lab3;


// ../../etc/passwd%00.txt
// ../../home/user/.ssh/id_rsa%00.txt